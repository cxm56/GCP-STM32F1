...
//SPI节点定义
SPI_TypeDef * const SPI[HAL_SPI_NODE_NUM] = { SPI1, SPI2, SPI3 };

//SPI 中断向量定义
static const IRQn_Type SPI_IRQn[HAL_SPI_NODE_NUM] = {SPI1_IRQn, SPI2_IRQn, SPI3_IRQn};
...
/**
  ...
  * @retval None
  */
void HAL_SPI_IOConfig(uint8_t uSpiNode, uint8_t uWorkMode)
{
  switch (uSpiNode)
  {
  case HAL_SPI_NODE0: 
      {
        //HAL_GPIO_ModeConfig(SPI1_NSS_PORT,  SPI1_NSS_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI1_CLK_PORT,  SPI1_CLK_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI1_MISO_PORT, SPI1_MISO_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_PUSH_UP : HAL_GPIO_AF_PP_H);
          HAL_GPIO_ModeConfig(SPI1_MOSI_PORT, SPI1_MOSI_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          break;
      }
      
  case HAL_SPI_NODE1: 
      {
        //HAL_GPIO_ModeConfig(SPI2_NSS_PORT,  SPI2_NSS_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI2_CLK_PORT,  SPI2_CLK_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI2_MISO_PORT, SPI2_MISO_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_PUSH_UP : HAL_GPIO_AF_PP_H);
          HAL_GPIO_ModeConfig(SPI2_MOSI_PORT, SPI2_MOSI_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          break;
      }
      
  case HAL_SPI_NODE2: 
      {
        //HAL_GPIO_ModeConfig(SPI3_NSS_PORT,  SPI3_NSS_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI3_CLK_PORT,  SPI3_CLK_PIN,  (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          HAL_GPIO_ModeConfig(SPI3_MISO_PORT, SPI3_MISO_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_PUSH_UP : HAL_GPIO_AF_PP_H);
          HAL_GPIO_ModeConfig(SPI3_MOSI_PORT, SPI3_MOSI_PIN, (uWorkMode == HAL_SPI_MASTER_MODE) ? HAL_GPIO_AF_PP_H : HAL_GPIO_FLOAT);
          break;
      }
...
/**
  ...
  * @retval None
  */
static void HAL_SPI_ModeConfig(uint8_t uSpiNode, uint8_t uWorkMode)
{
    //开时钟
    switch (uSpiNode)
    {
    case HAL_SPI_NODE0: RCC->APB2ENR |= RCC_APB2ENR_SPI1EN; break;
    case HAL_SPI_NODE1: RCC->APB1ENR |= RCC_APB1ENR_SPI2EN; break;
    case HAL_SPI_NODE2: RCC->APB1ENR |= RCC_APB1ENR_SPI3EN; break;
    default: break;
    }
    ...
    //从模式配置
    if (uWorkMode == HAL_SPI_SLAVE_MODE)
    {
        ...
        HAL_NVIC_Enable(SPI_IRQn[uSpiNode], 2, 2);  //开内核中断
...
/**
  ...
  * @retval None
  */
void HAL_SPI_Init(uint8_t uSpiNode, uint8_t uWorkMode)
{
    //配置工作模式
    HAL_SPI_ModeConfig(uSpiNode, uWorkMode);
    
    //配置引脚
    HAL_SPI_IOConfig(uSpiNode, uWorkMode);
    
    //配置工作速度
    HAL_SPI_SetSpeed(uSpiNode, HAL_SPI_SPEED_DIV16);
...
/**
  ...
  * @retval None
  */
void HAL_SPI_SetSpeed(uint8_t uSpiNode, uint8_t uSpeedDvi)
{
    //关闭SPIx
    SPI[uSpiNode]->CR1 &= ~SPI_CR1_SPE;
...
/**
  ...
  * @retval SR的值
  */
uint16_t HAL_SPI_GetStatus(uint8_t uSpiNode)
{
...
/**
  ...
  * @retval Note
  */
void HAL_SPI_WriteData(uint8_t uSpiNode, uint16_t nWriteData)
{
...
/**
  ...
  */
uint16_t HAL_SPI_ReadData(uint8_t uSpiNode)
{
...
/**
  ...
  * @retval 读取到的数据
  */
uint16_t HAL_SPI_ReadWriteByte(uint8_t uSpiNode, uint16_t nWriteData)
{
    uint8_t cReadData = 0;
