...
//定时器输入捕获中断向量定义
static const IRQn_Type TIM_IRQn[HAL_TIM_NODE_NUM] = {TIM1_CC_IRQn, TIM8_CC_IRQn, TIM2_IRQn, TIM3_IRQn, TIM4_IRQn, TIM5_IRQn};
...
/**
  ...
  * @retval None
  */
static void HAL_TIM_OutputConfig(uint8_t uTimeNode, uint8_t uChannelMask, uint8_t uOutputMode)
{
    ...
    //使能输出
    if((uTimeNode == HAL_TIM_0) || ((uTimeNode == HAL_TIM_1)))//假如是高级定时器,还需配置BDTR寄存器
    {
...
/**
  ...
  * @retval None
  */
void HAL_TIM_OutputInit(uint8_t uTimeNode, uint8_t uChannelMask, uint8_t uOutputMode)
{
    //引脚配置
    HAL_TIM_PortConfig(uTimeNode, uChannelMask, HAL_TIME_PORT_OUTPUT);
    
    //模式配置
    HAL_TIM_OutputConfig(uTimeNode, uChannelMask, uOutputMode);
    
    //时序配置
    HAL_TIM_SetOutputPwmDutyRatio(uTimeNode, uChannelMask, 50);
    
    //开启定时器
    HAL_TIM_OutputEnable(uTimeNode, 1);
...
/**
  ...
  * @retval None
  */
void HAL_TIM_EnableOutputIRQ(uint8_t uTimeNode, uint8_t uChannelMask, uint8_t bIsEnable)
{
    if (bIsEnable)
    {
        TIM[uTimeNode]->DIER |= (uChannelMask<<1); //开外设中断
        HAL_NVIC_Enable(TIM_IRQn[uTimeNode], 2, 2);//开内核中断
    }
    else 
    {
        HAL_NVIC_DisableIRQ(TIM_IRQn[uTimeNode]);
    }
...
/**
  ...
  * @retval None
  */
void HAL_TIM_SetOutputPwmDutyRatio(uint8_t uTimeNode, uint8_t uChannelMask, float fDutyRatio)
{
...
/**
  ...
  * @retval None
  */
void HAL_TIM_SetOutputCmpVal(uint8_t uTimeNode, uint8_t uChannelMask, uint16_t nCompareVal)
{
...
/**
  ...
  * @retval None
  */
void HAL_TIM_SetOutputPwmFrq(uint8_t uTimeNode, uint32_t ulFrequency)
{
...
/**
  ...
  * @retval None
  */
void HAL_TIM_SetOutputAutoReloadValue(uint8_t uTimeNode, uint16_t nReloadVal)
{
...
/**
  ...
  * @retval 定时器中的重加载值
  */
uint16_t HAL_TIM_GetOutputAutoReloadValue(uint8_t uTimeNode)
{
...
/**
  ...
  * @retval None
  */
void HAL_TIM_OutputEnable(uint8_t uTimeNode, bool bIsEnablle)
{
    if (bIsEnablle)
    {
