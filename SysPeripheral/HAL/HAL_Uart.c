...
static USART_TypeDef * const USART[HAL_UART_NODE_NUM] = {USART1,USART2, USART3, UART4};
static DMA_Channel_TypeDef * const USART_RxDmaChannel[HAL_UART_NODE_NUM] = {DMA1_Channel5, DMA1_Channel6, DMA1_Channel3, DMA2_Channel3};
static DMA_Channel_TypeDef * const USART_TxDmaChannel[HAL_UART_NODE_NUM] = {DMA1_Channel4, DMA1_Channel7, DMA1_Channel2, DMA2_Channel5};
static const IRQn_Type USART_IRQn[HAL_UART_NODE_NUM] = {USART1_IRQn, USART2_IRQn, USART3_IRQn, UART4_IRQn};
static const IRQn_Type USART_TX_DMA_IRQn[HAL_UART_NODE_NUM] = {DMA1_Channel4_IRQn, DMA1_Channel7_IRQn, DMA1_Channel2_IRQn, DMA2_Channel4_5_IRQn};
    ...
    case HAL_UART_NODE0: 
        {
            ...
            HAL_IRQ_SetTrgCallback(HAL_UART0_DMATC_IRQHandler, IRQ_TRG_DMA1_Channel4);
            break;
        }
    case HAL_UART_NODE1: 
        {
            HAL_IRQ_SetTrgCallback(HAL_UART1_RX_IRQHandler, IRQ_TRG_USART2);
            HAL_IRQ_SetTrgCallback(HAL_UART1_DMATC_IRQHandler, IRQ_TRG_DMA1_Channel7);
            break;
        }
    case HAL_UART_NODE2: 
        {
            HAL_IRQ_SetTrgCallback(HAL_UART2_RX_IRQHandler, IRQ_TRG_USART3);
            HAL_IRQ_SetTrgCallback(HAL_UART2_DMATC_IRQHandler, IRQ_TRG_DMA1_Channel2);
            break;
        }
    case HAL_UART_NODE3: 
        {
            HAL_IRQ_SetTrgCallback(HAL_UART3_RX_IRQHandler, IRQ_TRG_UART4);
            HAL_IRQ_SetTrgCallback(HAL_UART3_DMATC_IRQHandler, IRQ_TRG_DMA2_Channel5);
            break;
        }
...
static void HAL_UART_ModeConfig(uint8_t uUartNode, uint32_t ulBaudRate)
{
    ...
    switch (uUartNode)
    {
    case HAL_UART_NODE0: RCC->APB2ENR |= RCC_APB2ENR_USART1EN; break;
    case HAL_UART_NODE1: RCC->APB1ENR |= RCC_APB1ENR_USART2EN; break;
    case HAL_UART_NODE2: RCC->APB1ENR |= RCC_APB1ENR_USART3EN; break;
    case HAL_UART_NODE3: RCC->APB1ENR |= RCC_APB1ENR_UART4EN;  break;
    }
...
static void HAL_UART_DmaTxInit(uint8_t uUartNode)
{
    ...
    HAL_NVIC_Enable(USART_TX_DMA_IRQn[uUartNode], 2, 2); //开内核中断
...
static void HAL_UART0_RX_IRQHandler(void)
{
    if (USART[HAL_UART_NODE0]->SR & (0x1<<4))  //空闲帧中断入口
    {
        //清标志位
        USART[HAL_UART_NODE0]->DR;      //总线空闲中断标志位由软件序列清除:先读SR,再读DR
        
        //计算接收到的帧长
        uint16_t nRecvCnt = RBUF_SIZE - USART_RxDmaChannel[HAL_UART_NODE0]->CNDTR;
        
        //复位DMA
        HAL_UART_RxDMAReset(HAL_UART_NODE0);
        
        //数据帧处理
        RingBuffer_InsertMult(&m_RxRing[HAL_UART_NODE0], Uart0_RecvBuff, nRecvCnt);
    }
...
static void HAL_UART1_RX_IRQHandler(void)
{
    if (USART[HAL_UART_NODE1]->SR & (0x1<<4))  //空闲帧中断入口
    {
        //清标志位
        USART[HAL_UART_NODE1]->DR;      //总线空闲中断标志位由软件序列清除:先读SR,再读DR
        
        //计算接收到的帧长
        uint16_t nRecvCnt = RBUF_SIZE - USART_RxDmaChannel[HAL_UART_NODE1]->CNDTR;
        
        //复位DMA
        HAL_UART_RxDMAReset(HAL_UART_NODE1);
        
        //数据帧处理
        RingBuffer_InsertMult(&m_RxRing[HAL_UART_NODE1], Uart1_RecvBuff, nRecvCnt);
    }
...
static void HAL_UART2_RX_IRQHandler(void)
{
    if (USART[HAL_UART_NODE2]->SR & (0x1<<4))  //空闲帧中断入口
    {
        //清标志位
        USART[HAL_UART_NODE2]->DR;      //总线空闲中断标志位由软件序列清除:先读SR,再读DR
        
        //计算接收到的帧长
        uint16_t nRecvCnt = RBUF_SIZE - USART_RxDmaChannel[HAL_UART_NODE2]->CNDTR;
        
        //复位DMA
        HAL_UART_RxDMAReset(HAL_UART_NODE2);
        
        //数据帧处理
        RingBuffer_InsertMult(&m_RxRing[HAL_UART_NODE2], Uart2_RecvBuff, nRecvCnt);
    }
...
static void HAL_UART3_RX_IRQHandler(void)
{
    if (USART[HAL_UART_NODE3]->SR & (0x1<<4))  //空闲帧中断入口
    {
        //清标志位
        USART[HAL_UART_NODE3]->DR;      //总线空闲中断标志位由软件序列清除:先读SR,再读DR
        
        //计算接收到的帧长
        uint16_t nRecvCnt = RBUF_SIZE - USART_RxDmaChannel[HAL_UART_NODE3]->CNDTR;
        
        //复位DMA
        HAL_UART_RxDMAReset(HAL_UART_NODE3);
        
        //数据帧处理
        RingBuffer_InsertMult(&m_RxRing[HAL_UART_NODE3], Uart3_RecvBuff, nRecvCnt);
    }
